...
document.querySelectorAll('.counting').forEach(function (element) {
  ...
  function animate(startTime) {
    function step(currentTime) {
      ...
      const easedProgress = progress < 0.5 
        ...
        : 1 - Math.pow(-2 * progress + 2, 3) / 2; // Ease-out cubic easing
      ...
      element.textContent = allowDecimal 
        ...
        : Math.round(currentCount).toLocaleString(); // Форматування з/без десяткових частин
      ...
      if (progress < 1) {
        ...
        element.textContent = allowDecimal 
          ...
          : Math.round(countTo).toLocaleString(); // Переконайтесь, що фінальне значення задане точно
      }
    }
    requestAnimationFrame(step);
  }
  ...
  handleScroll(); // Перевірка одразу на випадок, якщо елемент уже в полі зору
});